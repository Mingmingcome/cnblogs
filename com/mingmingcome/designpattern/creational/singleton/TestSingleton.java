...
public class TestSingleton {
	...
	public static void main(String[] args) throws ClassNotFoundException, InterruptedException {
		...
		// 装Singleton的容器
		final Set<LazyInitThreadUnsafeSingleton> set0 = Collections.synchronizedSet(new HashSet<LazyInitThreadUnsafeSingleton>());
		final Set<LazyInitThreadSafeSingleton> set1 = Collections.synchronizedSet(new HashSet<LazyInitThreadSafeSingleton>());
		final Set<EagerSingleton> set2 = Collections.synchronizedSet(new HashSet<EagerSingleton>());
		final Set<DoubleCheckLockingSingleton> set3 = Collections.synchronizedSet(new HashSet<DoubleCheckLockingSingleton>());
		final Set<StaticInnerClassSingleton> set4 = Collections.synchronizedSet(new HashSet<StaticInnerClassSingleton>());
		final Set<EnumSingleton> set5 = Collections.synchronizedSet(new HashSet<EnumSingleton>());
		
		// 可重用的同步多个任务
		final CyclicBarrier cb0 = new CyclicBarrier(1000);
		...
		for (int i = 0; i < 1000; i++) {
			exec0.execute(new Runnable() {
				...
				public void run() {
					try {
						cb0.await();
						set5.add(EnumSingleton.getInstance());
					} catch (InterruptedException e) {
						e.printStackTrace();
					} catch (BrokenBarrierException e) {
						e.printStackTrace();
